...
class Solution:
    def cloneGraph(self, node: 'Node') -> 'Node':
        ...
        def findOrCreateNode(val):
            if val not in nodes:
                #print("Creating node ", val)
                nodes[val] = Node(val)
            return nodes[val]
        ...
        while Q:
            i = Q.popleft()
            #print("Parent ", i.val, "Adj: ", [j.val for j in i.neighbors])
            parent = findOrCreateNode(i.val)
            for j in i.neighbors:
                if j.val not in nodes:
                    Q.append(j)
                adj = findOrCreateNode(j.val)
                #print(parent.val, " - ", adj.val)
                parent.neighbors.append(adj)
