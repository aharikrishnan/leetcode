...
class Solution:
    def jobScheduling(self, startTime: List[int], endTime: List[int], profit: List[int]) -> int:
        ...
        for i in range(len(startTime)):
            T.append((startTime[i], i, False))
            T.append((endTime[i], i, True))
        T.sort(reverse=True)
        while T:
            t,i,isEnd = T.pop()
            if isEnd:
                lastProfit = max(lastProfit, profit[i] + dp[startTime[i]])
            lastProfit = dp[t] = max(dp.get(t, 0), lastProfit)
            res = max(res, lastProfit)
        return res 
